...
- 修改：
  ...
  - 新增 `RULE-SET,JPMustProxy,日本节点`，默认进入日本节点组；该组为可手动选择的 select 组，用户仍可选择具体代理节点。
  - 按用户追加需求新增空的 `USMustProxy` rule-provider 和 `ruleset/USMustProxy.list`，并新增 `RULE-SET,USMustProxy,美国节点`；默认进入美国节点组，该组同样可手动选择具体代理节点，列表暂未加入域名。
...
- 影响：`0-0.pro` 及其子域命中后默认进入“日本节点”组并可手动选择具体节点；`USMustProxy` 未来加入条目后默认进入“美国节点”组并可手动选择具体节点。现有 `MustProxy` 用户行为不变；`i00.pro` 继续强制直连。
- 撤回：否
- author: ai
- verified_by:
...
- 开始：2026-08-31 05:20 UTC (UTC+0)
- 结束：2026-08-31 05:22 UTC (UTC+0)
- 类型：规则增强 / DNS 配置
...
- 验证：`npm run build` 通过（tsgo --noEmit exit 0 + esbuild exit 0）；grep 确认 convert.min.js / convert.js 中 fake-ip-filter 数组已含 `+.llm-api.net`
- 影响：llm-api.net 及其子域名在 fake-ip 模式下返回真实 IP；MustDirect 直连规则保持不变；按用户要求未新增 AI_SERVICE 代理规则
- 撤回：否
- author: ai

### 2026-09-05 规则：MustDirect 新增 i00.pro 强制直连（0-0.pro 管理员提供的直连别名，有实测依据，sync-v2.7.0 同步版）

- 开始：2026-09-05
- 结束：2026-09-05
- 类型：规则 / 数据清单
- 对象：`ruleset/MustDirect.list`
- 原因：0-0.pro（境外聚合 API 平台）国内不可直连（见同日撤回条目），其管理员提供了别名域名 `i00.pro` 用于绕过 SNI 阻断。实测证据：i00.pro 直连 TLS 握手 ~0.87s 成功（0-0.pro 直连 TLS ~5s 失败，确认 GFW 黑名单只含 0-0.pro SNI）；`GET /v1/models`、`POST /v1/chat/completions`、`POST /v1/responses` 直连均 HTTP 200；偶发瞬时 503（重试即好）。DoH 解析 i00.pro 与 0-0.pro 同为 `157.245.196.149`（同一台 DigitalOcean 服务器，纯别名）。main 线同源 commit `6d8dbc1` cherry-pick 过来
- 修改：`ruleset/MustDirect.list` 末尾追加一行 `DOMAIN-SUFFIX,i00.pro`（匹配 i00.pro 及其所有子域；cherry-pick 自动套用，无冲突，CHANGELOG 手工解冲突）
- 验证：`git diff ruleset/MustDirect.list` 仅追加一行；两分支 MustDirect 内容一致；直连通路实测见上
- 影响：`i00.pro` 及其子域强制直连，不再依赖代理节点；main 线已即时生效（jsDelivr @main），本分支变更随下次 sync-v2.7.0 发版进入 convert.min.js
